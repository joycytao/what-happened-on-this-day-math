...
describe("Issue #48 regenerated October content", () => {
  ...
  it("uses the translated attachment question profile for every event", () => {
    assert.equal(content.contentProfile, "oct-content-example");
    for (const day of content.days) {
      for (const level of ["level1", "level2", "level3"]) {
        assert.equal(day.mathLevels[level].skill, "attachment-example-question");
        assert.doesNotMatch(day.mathLevels[level].prompt, /[\u3400-\u9fff]/);
      }
    }
  });
});
