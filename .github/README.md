...
- Agent pickup follows the explicit `[NN/Total]` title sequence, excludes `status: blocked`, and does not guess the order for Issues without a sequence marker.
- Issue number identifies the Issue but is not the execution-order key.
