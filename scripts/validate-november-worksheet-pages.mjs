...
if (!outputDirectory) {
  console.error("Usage: node scripts/validate-november-worksheet-pages.mjs <output-directory>");
  process.exit(2);
}
...
const expectedPageCount = expectedDayCount * expectedTypes.length;
const errors = [];
...
if (manifest.pageCount !== expectedPageCount || manifest.expectedPageCount !== expectedPageCount) errors.push(`manifest must declare ${expectedPageCount} pages`);
if (manifest.pages.length !== expectedPageCount) errors.push(`manifest page list must contain ${expectedPageCount} pages; found ${manifest.pages.length}`);

for (let index = 0; index < manifest.pages.length; index += 1) {
  const page = manifest.pages[index];
  const expectedDay = Math.floor(index / expectedTypes.length) + 1;
  ...
  if (svg.includes("2000-11-")) errors.push(`${page.date} ${page.pageType} exposes the synthetic renderer year`);
...
console.log(JSON.stringify({ valid: true, month: 11, expectedDayCount, pageCount: expectedPageCount, pageDimensions: { width: 1545, height: 2000 }, order: "day then reading-passage, level1, level2, level3" }, null, 2));
